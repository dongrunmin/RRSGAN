...
def vis_kernel(kernel):
    ...
    Args:
        kernel (Numpy, Tensor): [1]x[1]xHxW

    Returns:
        (Numpy): visualized kernel, [0, 255]
    """
    if type(kernel).__module__ == np.__name__:  # numpy type
        kernel = np.squeeze(kernel).astype(np.float32)
        kernel = kernel - np.min(kernel)
        kernel = kernel / np.max(kernel) * 255.
    else:  # Tensor type
        kernel = kernel.detach().cpu().squeeze().float()
        kernel = kernel - torch.min(kernel)
        kernel = kernel / torch.max(kernel) * 255.
        kernel = kernel.numpy()
    return kernel


def correlation(kernel1, kernel2):
    def reshape(kernel):
        H, W = kernel.shape
        return np.reshape(kernel, (H * W, ))

    kernel1 = reshape(kernel1)
    kernel2 = reshape(kernel2)
    kernel_m = np.stack([kernel1, kernel2], axis=0)
    cor_m = np.corrcoef(kernel_m)
    return cor_m[0][1]
