...
def tensor2img_norm(tensor, out_type=np.uint8, min_max=(0, 255)):
    '''
    Converts a torch Tensor into an image Numpy array
    Input: 4D(B,(3/1),H,W), 3D(C,H,W), or 2D(H,W), any range, RGB channel order
    Output: 3D(H,W,C) or 2D(H,W), [0,255], np.uint8 (default)
    ...
    compress_scale = (max_nub-min_nub)

    tensor = (tensor-min_nub)/compress_scale
...
def vis_attention_map(tensor, out_type=np.uint8):
    '''
    Converts a feature map into an attention map
    '''
